...
public class DatabaseManager extends SQLiteOpenHelper {
    ...
    public void insertMessage(String messageID, String sender, String message) {
        ...
        values.put(COLUMN_MESSAGEID, messageID);
    ...
    public boolean checkDuplicateEntry(String messageID) { //Return true for duplicate, false for no duplicate
        String dbString = "";
        SQLiteDatabase db = getWritableDatabase();
        String query = "SELECT " + COLUMN_MESSAGEID + " FROM " + TABLE_MESSAGE + " WHERE " + COLUMN_MESSAGEID + " = '" + messageID + "'";
        Cursor cursor = db.rawQuery(query, null);
        if (cursor.moveToFirst()) {  //Returns true if item already exists
            db.close();
            return true;
